import { useEffect } from "react";
import { useForm, Controller } from "react-hook-form";
import { z } from "zod";
import { zodResolver } from "@hookform/resolvers/zod";
import { useMembership, useUpdateMembership } from "../api";
import { useParams } from "@tanstack/react-router";

const SERVICES = [
	{ id: 1, name: "Personal Training" },
	{ id: 2, name: "Massage Therapy" },
	{ id: 3, name: "Sauna Access" },
	{ id: 4, name: "Pool Access" },
	{ id: 5, name: "Group Fitness Classes" },
	{ id: 6, name: "Yoga Classes" },
	{ id: 7, name: "Pilates Classes" },
	{ id: 8, name: "Nutrition Consultation" },
] as const;

const schema = z.object({
	name: z.string().min(3, "Min 3 chars"),
	price: z.number().positive(),
	duration_months: z.number().int().min(1),
	status: z.string().nullable().optional(),
	serviceIds: z.array(z.number()).default([]).optional(),
});

type FormInputs = z.infer<typeof schema>;

export const MembershipEdit = () => {
	const { membershipsId } = useParams({ from: "/memberships/$membershipsId" });

	const { data: membership, isLoading: isLoadingMembership } =
		useMembership(membershipsId);
	const { mutate: update, isPending, isError, error } = useUpdateMembership();

	const {
		control,
		register,
		reset,
		handleSubmit,
		formState: { errors },
	} = useForm<FormInputs>({
		resolver: zodResolver(schema),
		defaultValues: {
			name: "",
			price: 0,
			duration_months: 1,
			serviceIds: [],
			status: null,
		},
	});

	useEffect(() => {
		if (membership) {
			reset({
				name: membership.membershipName,
				price: membership.price,
				duration_months: membership.durationMonths,
				status: membership.status ?? "",
				serviceIds: membership.services?.map((s: any) => s.id) ?? [],
			});
		}
	}, [membership, reset]);

	const onSubmit = (data: FormInputs) => {
		update({
			id: membershipsId,
			data: {
				name: data.name,
				price: data.price,
				duration_months: data.duration_months,
				status: data.status ?? null,
				serviceIds: data.serviceIds,
			},
		});
	};

	if (isLoadingMembership) {
		return <p className="text-center py-6">Loading...</p>;
	}

	return (
		<div className="max-w-lg mx-auto p-6">
			<h1 className="text-2xl font-semibold mb-4">Update membership</h1>

			<form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
				{/* NAME */}
				<div>
					<label className="block text-sm mb-1">Name</label>
					<input
						type="text"
						{...register("name")}
						className="w-full rounded-md border px-3 py-2"
					/>
					{errors.name && (
						<p className="text-red-600 text-sm">{errors.name.message}</p>
					)}
				</div>

				{/* PRICE */}
				<div>
					<label className="block text-sm mb-1">Price</label>
					<input
						type="number"
						step="0.01"
						{...register("price", { valueAsNumber: true })}
						className="w-full rounded-md border px-3 py-2"
					/>
					{errors.price && (
						<p className="text-red-600 text-sm">{errors.price.message}</p>
					)}
				</div>

				{/* DURATION */}
				<div>
					<label className="block text-sm mb-1">Duration (months)</label>
					<input
						type="number"
						min={1}
						{...register("duration_months", { valueAsNumber: true })}
						className="w-full rounded-md border px-3 py-2"
					/>
					{errors.duration_months && (
						<p className="text-red-600 text-sm">
							{errors.duration_months.message}
						</p>
					)}
				</div>

				{/* STATUS */}
				<div>
					<label className="block text-sm mb-1">Status</label>
					<select
						{...register("status")}
						className="w-full rounded-md border px-3 py-2"
					>
						<option value="">—</option>
						<option value="active">active</option>
						<option value="inactive">inactive</option>
					</select>
				</div>

				{/* SERVICES CHECKBOXES */}
				<div>
					<label className="block text-sm mb-2">Services</label>
					<Controller
						control={control}
						name="serviceIds"
						render={({ field }) => (
							<div className="grid grid-cols-2 gap-2">
								{SERVICES.map((s) => {
									const checked = field.value?.includes(s.id) ?? false;

									return (
										<label
											key={s.id}
											className="flex items-center gap-2 text-sm"
										>
											<input
												type="checkbox"
												checked={checked}
												onChange={() => {
													if (checked) {
														field.onChange(
															field?.value?.filter((v: any) => v !== s.id)
														);
													} else {
														field.onChange([...field.value!, s.id]);
													}
												}}
											/>
											{s.name}
										</label>
									);
								})}
							</div>
						)}
					/>
				</div>

				{/* SUBMIT */}
				<button
					type="submit"
					disabled={isPending}
					className="w-full rounded-md bg-indigo-600 px-4 py-2 text-white"
				>
					{isPending ? "Saving..." : "Save changes"}
				</button>

				{isError && (
					<p className="text-sm text-red-600 mt-2">
						{error instanceof Error ? error.message : "Failed to update"}
					</p>
				)}
			</form>
		</div>
	);
};
